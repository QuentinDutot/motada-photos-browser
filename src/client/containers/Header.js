...
const Header = () => {
  ...
  useEffect(() => {
    axios('/api/images?count').then(response => setCount(response.data.count))
  }, [])

  const openGithubProjet = () => {
  ...
  return (
    <div className="w-full flex items-center justify-center">
      <div className="lg:w-3/5 md:w-4/5 w-full relative md:mx-32 md:my-28 mx-1 my-16">
        ...
        <div className="flex items-center md:justify-between justify-center mb-8">
          ...
          <div className="md:flex hidden flex-wrap items-center">
            ...
            <svg
              width="24"
              height="24"
              className="bg-white rounded-full shadow cursor-pointer"
              onClick={openGithubProjet}
            >
        ...
        <Translate open={dialog} close={() => setDialog(false)} />
      </div>
    </div>
  )
}
